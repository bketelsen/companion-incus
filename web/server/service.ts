...
const LABEL = "com.companion-incus";
...
const UNIT_NAME = "companion-incus.service";
...
export function generateSystemdUnit(opts: UnitOptions): string {
  ...
  return `[Unit]
Description=Companion Incus - Web UI for Claude Code
...
function resolveBinPath(): string {
  try {
    const binPath = execSync("which companion-incus", { encoding: "utf-8" }).trim();
    if (binPath) return binPath;
  } catch {
    // not found globally
  }

  console.error("companion-incus must be installed globally for service mode.");
  console.error("");
  console.error("  bun install -g companion-incus");
  console.error("");
  console.error("Then retry:");
  console.error("");
  console.error("  companion-incus install");
  process.exit(1);
}
...
async function installDarwin(opts?: { port?: number }): Promise<void> {
  ...
  if (existsSync(PLIST_PATH)) {
    console.error("Companion Incus is already installed as a service.");
    console.error("Run 'companion-incus uninstall' first to reinstall.");
    process.exit(1);
  }
  ...
  console.log("Companion Incus has been installed as a background service.");
  console.log("");
  ...
  console.log("Use 'companion-incus status' to check if it's running.");
}
...
async function uninstallDarwin(): Promise<void> {
  ...
  console.log("Companion Incus service has been removed.");
...
async function startDarwin(): Promise<void> {
  ...
  if (!installedService) {
    console.log("Companion Incus is not installed as a service.");
    console.log("Run 'companion-incus install' first.");
    return;
  }
  ...
  console.log("Companion Incus service has been started.");
}
...
async function stopDarwin(): Promise<void> {
  ...
  console.log("Companion Incus service has been stopped.");
  console.log("Run 'companion-incus restart' to start it again.");
}
...
async function restartDarwin(): Promise<void> {
  ...
  console.log("Companion Incus service has been restarted.");
}
