...
function plistPath(): string {
  return join(tempDir, "Library", "LaunchAgents", "com.companion-incus.plist");
}
...
function unitPath(): string {
  return join(tempDir, ".config", "systemd", "user", "companion-incus.service");
}
...
describe("generatePlist", () => {
  it("generates valid XML with the correct label", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/companion-incus" });
    expect(plist).toContain('<?xml version="1.0"');
    expect(plist).toContain("<string>com.companion-incus</string>");
  });
  ...
  it("uses the provided binary path in ProgramArguments", () => {
    const plist = service.generatePlist({ binPath: "/opt/homebrew/bin/companion-incus" });
    expect(plist).toContain("<string>/opt/homebrew/bin/companion-incus</string>");
  ...
  it("uses a custom port when specified", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/companion-incus", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/companion-incus", path: customPath });
...
describe("generateSystemdUnit", () => {
  it("generates a valid systemd unit with correct sections", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/companion-incus" });
  ...
  it("includes the description", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/companion-incus" });
    expect(unit).toContain("Description=Companion Incus");
  });

  it("uses the provided binary path in ExecStart", () => {
    const unit = service.generateSystemdUnit({ binPath: "/home/user/.bun/bin/companion-incus" });
    expect(unit).toContain("ExecStart=/home/user/.bun/bin/companion-incus start --foreground");
  });
  ...
  it("uses a custom port when specified", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/companion-incus", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    const customPath = "/custom/bin:/other/bin";
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/companion-incus", path: customPath });
...
describe("install", () => {
  it("creates log directory and writes plist file", async () => {
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/usr/local/bin/companion-incus\n";
    ...
    expect(content).toContain("com.companion-incus");
  });
  ...
  it("migrates old launchd label before installing", async () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/companion-incus" })
      .replaceAll("com.companion-incus", "sh.thecompanion.app");
...
describe("install (linux)", () => {
  ...
  it("creates log directory and writes systemd unit file", async () => {
    ...
    expect(content).toContain("ExecStart=/usr/local/bin/companion-incus start --foreground");
  });
...
describe("start (linux)", () => {
  ...
  it("calls systemctl start when installed", async () => {
    ...
    const startCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("start companion-incus.service"),
    );
    expect(startCall).toBeDefined();
  });
...
describe("stop (linux)", () => {
  ...
  it("calls systemctl stop when installed", async () => {
    ...
    const stopCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("stop companion-incus.service"),
    );
    expect(stopCall).toBeDefined();
  });
...
describe("restart (linux)", () => {
  ...
  it("calls systemctl restart when installed", async () => {
    ...
    const restartCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("restart companion-incus.service"),
    );
    expect(restartCall).toBeDefined();
  });
...
describe("status (linux)", () => {
  ...
  it("returns installed: true, running: true with PID", async () => {
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (typeof cmd === "string" && cmd.includes("show companion-incus.service")) {
...
describe("refreshServiceDefinition (macOS)", () => {
  it("rewrites plist with current binary path", async () => {
    ...
    expect(originalContent).toContain("/usr/local/bin/companion-incus");
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/new/path/companion-incus\n";
      return "";
    });
    ...
    expect(updatedContent).toContain("/new/path/companion-incus");
  });
