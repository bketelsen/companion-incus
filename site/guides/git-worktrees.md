...
Companion Incus lets you select a git branch when creating a session and optionally create an isolated git worktree so multiple agents can work on the same repo without interfering with each other.
...
1. Companion Incus creates a new working directory at `~/.companion/worktrees/{repo-name}/{branch}/`
...
::: info
Worktrees are not needed with Incus containers. Containers provide full filesystem isolation — the host workspace is copied into `/workspace` inside the container.
:::
